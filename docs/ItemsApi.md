...
[**remove_item_from_set**](ItemsApi.md#remove_item_from_set) | **DELETE** /item_sets/{item_set_id}/items/{id} | Remove Item from Set
...
> remove_item_from_set(item_set_id, id, content_type=content_type)
...
async with sparkfly.ApiClient(configuration) as api_client:
    ...
    try:
        # Remove Item from Set
        await api_instance.remove_item_from_set(item_set_id, id, content_type=content_type)
    except Exception as e:
        print("Exception when calling ItemsApi->remove_item_from_set: %s\n" % e)
```
